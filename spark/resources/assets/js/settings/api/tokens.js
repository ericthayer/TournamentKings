module.exports = {
    ...
    methods: {
         ...
         */
        updateToken() {
            Spark.put(`/settings/api/token/${this.updatingToken.id}`, this.updateTokenForm).then(response => {
        ...
        toggleAbility(ability) {
            if (this.abilityIsAssigned(ability)) {
                this.updateTokenForm.abilities = _.reject(this.updateTokenForm.abilities, a => a == ability);
            } else {
