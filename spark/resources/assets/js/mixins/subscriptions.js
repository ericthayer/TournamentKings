...
module.exports = {
    ...
    methods: {
        ...
        updateSubscription(plan) {
            ...
            axios
                .put(this.urlForPlanUpdate, { plan: plan.id })
                .then(() => {
                    Bus.$emit('updateUser');
                    Bus.$emit('updateTeam');
                })
    ...
    computed: {
        ...
        trialEndsAt() {
            ...
            if (this.onGenericTrial) {
                return moment
                    .utc(this.billable.trial_ends_at)
                    .local()
                    .format('MMMM Do, YYYY');
            }

            return moment
                .utc(this.activeSubscription.trial_ends_at)
                .local()
                .format('MMMM Do, YYYY');
        },
        ...
        needsSubscription() {
            return (
                !this.activeSubscription ||
                (this.activeSubscription.ends_at && moment.utc().isAfter(moment.utc(this.activeSubscription.ends_at)))
            );
        },
