<template>
    <div>
        <checkbox-with-label class="m-2" :checked="isChecked" @change="updateCheckedState(option.value, $event)">
            {{ option.name }}
        </checkbox-with-label>
    </div>
</template>
...
export default {
    ...
    computed: {
        isChecked() {
            return (
                this.$store.getters[`${this.resourceName}/filterOptionValue`](this.filter.class, this.option.value) ==
                true
            );
        },
    },
